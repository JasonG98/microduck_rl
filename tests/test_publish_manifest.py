"""`uv run publish` 写入 microduck 守护进程所加载的内容 - schema 2, 上传前先完成校验.

守护进程 (`pollen-robotics/microduck`) 会拒绝 manifest 与它的 `duck_ipc_proto` 常量
不一致的策略, 拒绝加载非 61 -> 14 的图, 并且只会把恒定命令的 `episodic` 条目
变成技能. 这些测试固定在 CPU 上、不依赖 mjlab 时, 本侧写入的正是如此.
"""
...
# `RemiFabre/microduck-flamingo-cycle` 发布时的 manifest - 这是此 schema 必须保持
# 兼容的社区约定, 除精简后的说明文字外逐字保留.
FLAMINGO = {
    "schema_version": 2,
    "model_api": 1,
    "name": "flamingo-cycle",
...
def _tiny_policy(path: Path, obs_len: int = m.OBS_LEN, action_len: int = m.ACTION_LEN) -> Path:
    """一个单层 '策略', 具有守护进程期望的形状, 这样 ONNX 检查无需 torch 也能运行."""
...
def test_constants_are_the_daemons():
    """`duck_ipc_proto`: POLICY_OBS_LEN 61, POLICY_ACTION_LEN 14, ROBOT_MODEL microduck. 这里
    一旦偏离, 每个机器人在下载前都会被拒绝."""
...
def test_a_gait_is_perpetual_with_nothing_to_unwind():
    """行走策略同样属于 perpetual, 并放入一个 slot - 无 hold, 无 unwind, 无 skill."""
...
def test_the_cli_dry_run_writes_a_repo(tmp_path, monkeypatch):
    """无需 Hub 或 GPU 的端到端: 输入一个 ONNX, 输出三个 repo 文件."""
