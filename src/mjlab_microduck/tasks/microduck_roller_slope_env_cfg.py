...
Le robot spawne sur du plat (impulsion vers l'avant), roule sur une rampe descendante et se laisse glisser en restant
debout. Aucun pilotage : la commande twist est neutralisée (rel_standing_envs=1.0). Terrain custom plat+rampe
(FlatRampTerrainCfg), curriculum de raideur (terrain_levels_slope). Obs 61D unifié → interchangeable au runtime (--new-
cmd-obs) — hérité tel quel de make_microduck_velocity_rollers_env_cfg (DR/obs/reset non touchés ici).
"""
...
RAMP_LENGTH_RANGE = (
    3.0,
    8.0,
)  # longueur horizontale de la rampe, tirée au hasard par tuile
...
def make_microduck_roller_slope_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    """Build the roller slope descent env cfg (flat + ramp + runout, twist neutralized)."""
