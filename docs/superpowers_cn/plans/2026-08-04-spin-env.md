...
> **面向 agent 工作者:** 必读子技能: 使用 superpowers:subagent-driven-development(推荐)或 superpowers:executing-plans 逐任务实施本计划。使用复选框(`- [ ]`)语法跟踪每个步骤。

**目标:** 新增一个 RL 任务 `Mjlab-Spin-Flat-MicroDuck`, 让装在滚轮上的 microduck 学会在约 6 rad/s 的速度下原地逆时针旋转约 2 圈后停下。这是一个受 runtime 按钮槽相位驱动的周期性动作。

**架构:** 新的 env cfg 克隆自 `microduck_roller_crouch_env_cfg.py` 的结构(滚轮机器人、61 维 obs、完整 DR、`GroundPickPhaseCommand`), 但用**结果驱动**的 reward 替代 pose reward(在相位上跟踪梯形目标偏航速度), 外加两个衰减的 shaping 引导项把动作推向差速滚动。所有新 reward 函数都放进 `src/mjlab_microduck/tasks/mdp.py`, 每个函数切成一个**纯值函数**(无需模拟器即可测试)+ 一个 **env wrapper —— 这正是仓库里已有的惯用法(`crouch_glide_reward_from_values`)。

**技术栈:** Python 3.12, mjlab 1.3.0, MuJoCo / mujoco-warp, torch, rsl_rl, pytest, uv。

**参考 Spec:** `docs/superpowers/specs/2026-08-04-spin-env-design.md`

## 全局约束

- 所有包络数值都由 spec 固定: `SPIN_PERIOD = 4.0` s, `SPIN_RATE_MAX = 6.0` rad/s, `SPIN_ACCEL_END = 0.125`, `SPIN_HOLD_END = 0.525`, `SPIN_BRAKE_END = 0.650`。未经 spec 变更不得更改。
- 旋转方向: **仅逆时针**, 目标 ω_z **为正**。
- `ENABLE_SYMMETRY = False` 且 `symmetry_cfg=None`: 左右对称增强会破坏单一方向的旋转。
- `angular_momentum`(角动量三维范数)reward 必须**不存在**于该 env: 它会对抗旋转。
- actor 观测必须保持 **61 维**(与 roller / ground_pick / crouch 布局一致), 否则 ONNX 无法在 runtime 槽中加载。
- 关节**始终**通过 `asset.find_joints(...)` 按名称/正则解析, 绝不用硬编码索引: 滚轮模型的关节顺序中插着 4 个被动轮。
- 入口速度通过 `cfg.events["reset_base"].params["velocity_range"]` 注入, **绝不**通过 `mode="reset"` 的 `push_by_setting_velocity`(已知 NaN 回归)。
- 仓库风格: env cfg 中的注释用法语, `mdp.py` 中函数 docstring 视上下文用英语或法语, commit 中不写 `Co-Authored-By`。
...
两个纯数学函数承载了该动作的完整定义: 目标偏航速度曲线和由其推导出的 shaping 门控。

**Files:**
- 修改: `src/mjlab_microduck/tasks/mdp.py`(追加到文件末尾, 位于 `RelativeHeadingVelocityCommandCfg` 和打滑 reward 之后)
- 测试: `tests/test_spin.py`(新建)

**接口:**
- 依赖: 无(作用于 `torch.Tensor` 的纯函数)
- 产出:
...
执行: `uv run --with pytest pytest tests/test_spin.py -q`
预期: 失败, 报错 `AttributeError: module 'mjlab_microduck.tasks.mdp' has no attribute 'spin_rate_by_phase'`

- [ ] **步骤 3: 实现这两个函数**

追加到 `src/mjlab_microduck/tasks/mdp.py` 末尾:
...
### 任务 2: 偏航跟踪与「原地」rewards

主要目标(`spin_rate_track`)、它的 L1 引导项, 以及让机器人保持在原地的惩罚项。本任务还引入**供任务 2 到 4 测试共享的假 env**, 从而无需启动 MuJoCo 即可测试 env wrappers。

**Files:**
- 修改: `src/mjlab_microduck/tasks/mdp.py`(接在任务 1 之后)
- 修改: `tests/test_spin.py`

**接口:**
- 依赖: `spin_rate_by_phase`, `spin_gate_by_phase`, `SPIN_*`(任务 1)
- 产出:
...
追加到 `tests/test_spin.py`(该假 env 也用于任务 3 和 4):
...
执行: `uv run --with pytest pytest tests/test_spin.py -q`
预期: 失败 —— `AttributeError: ... has no attribute 'spin_phase_from_command'`(任务 1 的 6 个测试仍然通过)

- [ ] **步骤 3: 实现 rewards**

在 `src/mjlab_microduck/tasks/mdp.py` 中接续追加:
...
两个注入已知物理规律的 rewards: 两条滑靴反向滚动(`spin_wheel_differential`)以及两片刀片保持贴地(`spin_grounded`)。两者都由门控 `gate(φ)` 承载。

**符号提醒**(spec 中已推导): 对逆时针旋转, 左滑靴朝**后**、右滑靴朝**前**; 4 个轮子在前进时都取正转, 因此 **`ω_D − ω_G > 0`**。

**Files:**
- 修改: `src/mjlab_microduck/tasks/mdp.py`(接在任务 2 之后)
- 修改: `tests/test_spin.py`

**接口:**
- 依赖: `_spin_gate`, `SPIN_*`(任务 1-2), `_FakeEntity` / `_FakeSensor` / `_FakeEnv`(任务 2)
- 产出:
...
执行: `uv run --with pytest pytest tests/test_spin.py -q`
预期: 失败 —— `AttributeError: ... has no attribute 'spin_wheel_differential'`
...
- [ ] **步骤 5: 测量真实半轮距并调整 `omega_scale`**

spec 将 `omega_scale = 20.0` 建立在**估计**的 0.03 m 半轮距上。在 HOME 位姿下测量滚轮模型上的真实值:
...
如果期望差分值与 20.0 相差超过 30%, 则将 `SPIN_WHEEL_OMEGA_SCALE` 设为测量值(四舍五入为整数)并在常量上方的注释中记录该测量值。否则保留 20.0 并在注释中记录测量值。无论哪种情况, 注释都必须包含测量到的半轮距。

- [ ] **步骤 6: 重新运行测试**

执行: `uv run --with pytest pytest tests/test_spin.py -q`
预期: 通过(18 个测试 —— 测试显式传入 `omega_scale=20.0`, 因此它们与常量的取值无关)
...
位姿 shaping(`leg_antisymmetry`)以及对 `neck_joint_pos_l2` 的调整, 让 `head_yaw` 自由作为惯性飞轮。

**符号陷阱**: 机器人采用左右**镜像**符号约定。一个*对称*位姿满足 `q_G + q_D ≈ 0`(这正是 `leg_symmetry_reward` 所测量的)。因此**剪刀步**(一条腿向前、另一条向后)满足 `q_G ≈ q_D`, 用 `−|q_G − q_D|` 度量。

**Files:**
- 修改: `src/mjlab_microduck/tasks/mdp.py`(`neck_joint_pos_l2`, 约第 1237 行处; 随后接在任务 3 之后追加)
- 修改: `tests/test_spin.py`

**接口:**
- 依赖: `_spin_gate`, `SPIN_*`, `_FakeEntity` / `_FakeEnv`
- 产出:
  ...
  - `neck_joint_pos_l2(env, asset_cfg=_NECK_JOINT_CFG, pattern: str = r".*(neck|head).*") -> torch.Tensor`(签名**扩宽**, 默认行为不变)
...
执行: `uv run --with pytest pytest tests/test_spin.py -q`
预期: 失败 —— `AttributeError: ... has no attribute 'leg_antisymmetry'`

- [ ] **步骤 3: 给 `neck_joint_pos_l2` 增加 `pattern` 参数**

在 `src/mjlab_microduck/tasks/mdp.py` 中, 将现有函数(约第 1237 行)替换为:
...
- [ ] **步骤 4: 实现 `leg_antisymmetry`**

在 `src/mjlab_microduck/tasks/mdp.py` 中接在任务 3 之后追加:
...
`neck_joint_pos_l2` 被 roller / slope / swizzle envs 使用; 它的签名已变更(新增带默认值的参数, 因此兼容)。

执行: `uv run --with pytest pytest tests/ -q`
预期: 通过 —— 无既有测试被破坏
...
**接口:**
- 依赖: `spin_rate_track`, `spin_rate_l1`, `spin_stay_in_place`, `spin_wheel_differential`, `spin_grounded`, `leg_antisymmetry`, `neck_joint_pos_l2(pattern=...)`, `SPIN_PERIOD`, `SPIN_RATE_MAX`, `SPIN_ACCEL_END`, `SPIN_HOLD_END`, `SPIN_BRAKE_END`(任务 1-4)
- 产出:
  - `make_microduck_spin_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg`
  - `MicroduckSpinRlCfg: RslRlOnPolicyRunnerCfg`
  - 任务 id `Mjlab-Spin-Flat-MicroDuck`

- [ ] **步骤 1: 编写会失败的测试**

新建 `tests/test_spin_cfg.py`:
...
执行: `uv run --with pytest pytest tests/test_spin_cfg.py -q`
预期: 失败, 报错 `ModuleNotFoundError: No module named 'mjlab_microduck.tasks.microduck_spin_env_cfg'`
...
在 `src/mjlab_microduck/tasks/__init__.py` 中, 在 `microduck_roller_slope_env_cfg` 块之后(约第 66 行)添加 import:
...
然后在文件末尾, 在 `Mjlab-RollerSlope-Flat-MicroDuck` 块之后注册:
...
执行: `uv run python -c "import mjlab_microduck.tasks"`
预期: 输出包含 `✓ Spin task registered: Mjlab-Spin-Flat-MicroDuck`, 且无异常
...
单元测试并不能证明 env 真的能运行: 观测的 61 个维度、接触传感器、关节解析以及无 NaN 现象, 只有在实际启动模拟器时才能看到。本任务是本计划的出口关口。

**Files:**
- 修改: `docs/superpowers/specs/2026-08-04-spin-env-design.md`(记录半轮距测量值与 smoke run 结果)

**接口:**
- 依赖: 已注册任务 `Mjlab-Spin-Flat-MicroDuck`(任务 5)
- 产出: 无代码 —— 一条验证与一条记录

- [ ] **步骤 1: 验证 actor obs 的真实维度**
...
"
```
预期: `OK: layout obs actor identique au crouch`。如果列表不同, 先修正 env cfg 的 OBSERVATIONS 块再继续 —— 这是 ONNX 能在 runtime 槽中加载的前提条件。

- [ ] **步骤 2: 启动一个带 NaN 防护的极短训练**
...
uv run train Mjlab-Spin-Flat-MicroDuck \
  ...
  --enable-nan-guard
```
预期: 5 次迭代无异常、无 NaN, 且日志 `Episode_Reward/` 中确实列出 `spin_rate_track`, `spin_rate_l1`, `spin_stay_in_place`, `spin_wheel_differential`, `spin_grounded`, `leg_antisymmetry`。

如果出现 NaN: 转储在 `/tmp/mjlab/nan_dumps/`。头号嫌疑是入口速度 —— 确认它正确通过 `reset_base.velocity_range` 注入, 而不是通过 `mode="reset"` 的 push。

- [ ] **步骤 3: 启动一次标定训练(500 次迭代)**

执行:
```bash
uv run train Mjlab-Spin-Flat-MicroDuck --env.scene.num-envs 4096 --agent.max_iterations 500
```
预期: `Episode_Reward/spin_rate_track` 在 500 次迭代中**上升**。这是目标可学的信号。记录达到的值。

如果曲线保持平坦, 按顺序应用 spec 中的「Plan B」:(1) 速度课程 3 → 6 rad/s, (2) 调高 `spin_wheel_differential` 并推迟 `leg_antisymmetry` 的衰减, (3) 将 `std` 从 1.5 放宽到 2.5。

- [ ] **步骤 4: 观察动作**

执行: `uv run scripts/play_latest.py`
预期: 机器人尝试原地逆时针旋转。500 次迭代时动作会比较粗糙; 我们要确认的是它朝**正确方向**旋转, 不产生平移, 也不总是以摔倒收场。

- [ ] **步骤 5: 在 spec 中记录测量值**

在 `docs/superpowers/specs/2026-08-04-spin-env-design.md` 末尾添加 `## Résultats de la vérification initiale` 小节, 包含: 测量到的半轮距(任务 3 步骤 5)、`omega_scale` 采用的取值、5 次迭代 smoke run 的结果, 以及 500 次迭代时 `Episode_Reward/spin_rate_track` 的取值。
...
- **必须遵循的顺序**: 任务 1 → 2 → 3 → 4 → 5 → 6。任务 2 到 4 依赖在任务 2 中创建的假 env; 任务 5 消费任务 1 到 4 的所有函数。
- **本任务的第 1 大陷阱**是 `angular_momentum` reward: 如果它留在 env 中, 会惩罚角动量的三维范数, spin 将永远学不出来。测试 `test_angular_momentum_reward_is_removed` 守护着这一点。
- **第 2 大陷阱**是机器人左右镜像的符号约定: 剪刀步用 `|q_G − q_D|` 度量, 而不是 `|q_G + q_D|`。有疑问时对照 `leg_symmetry_reward`(位于 `mdp.py` 约第 3769 行)。
- **第 3 大陷阱**是 61 维 obs 的一致性: 任何与 crouch 布局的偏差都会让 ONNX 在 runtime 槽中无法使用。任务 6 步骤 1 对此进行显式验证。
- 无论多么想加速学习, 都不要激活 PPO 对称性。
- **本计划之外的范围**: 完整训练(8000 次迭代)、ONNX 导出以及 runtime 槽部署。相关命令在 spec 中; 本计划在 env 验证为可学后即告结束(任务 6)。