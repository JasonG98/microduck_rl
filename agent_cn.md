...
当用户要求学习本项目, 继续课程, 复习理解, 跟踪进度或整理学习笔记时,
读取并使用 [microduck-mentor](.agents/skills/microduck-mentor/SKILL.md).
先读 [学习进度](docs/learning/progress.md) 和其中链接的最近记录,
结合 [课程计划](docs/learning/plan.md) 选择下一小节.
用户有 CV 从业经验和基本 RL 理论, 缺少 RL 项目实操; 优先理解工程结构
和执行数据流, 再进入实验. 实质学习回合应记录到仓库, 分清已讲解,
已实践和用户已经证明掌握的内容. 此流程只针对学习请求, 不把所有编码
或训练任务变成课程. 不自动恢复用户已搁置的话题. 接续依据是仓库文件,
不是聊天历史或某台机器的个人记忆.

本节于 2026-09-05 与英文源文件同步; 其余内容沿用文件头标注的同步版本.
